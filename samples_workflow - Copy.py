...
async def upload_worksheet_file(
    ...
    TARGET_BUCKET = "worksheets"
    
    try:
        # 1. Fetch worksheet info
        cur.execute("""
        ...
        # 2. Sanitize item_code to prevent folder creation
        safe_item_code = (item_code or 'worksheet').replace('/', '_').replace('\\', '_').replace(' ', '_')
        cloud_filename = f"WS_{worksheet_no}_{safe_item_code}_{worksheet_id}{file_ext}"
        
        # 3. Upload file - NO FOLDER PREFIXES
        try:
            print(f"📤 Uploading to bucket: {TARGET_BUCKET}")
            
            # Remove old version if exists (optional)
            try:
                supabase.storage.from_(TARGET_BUCKET).remove([cloud_filename])
            except:
                pass
            
            # PRIMARY UPLOAD - path is just filename, no folder prefixes
            supabase.storage.from_(TARGET_BUCKET).upload(
                path=cloud_filename,
                file=file_content,
                file_options={
                    "content-type": worksheet_file.content_type or "application/octet-stream", 
                    "x-upsert": "true"
                }
            )
            
            # Generate URL from the SAME bucket
            public_url = supabase.storage.from_(TARGET_BUCKET).get_public_url(cloud_filename)

        except Exception as e:
            print(f"❌ Primary upload failed: {str(e)}. Retrying with simple name...")
            # FALLBACK: Simpler filename, still NO folder prefixes
            simple_filename = f"worksheet_{worksheet_no}_{secrets.token_hex(4)}{file_ext}"
            ...
            public_url = supabase.storage.from_(TARGET_BUCKET).get_public_url(simple_filename)
            cloud_filename = simple_filename

        # 4. Update database with the URL
        print(f"💾 Saving to DB: {public_url}")
        cur.execute("""
            UPDATE worksheets 
            SET template_path = %s,
                updated_at = NOW(),
...
@router.get("/worksheets/{worksheet_id}/download")  # Fixed: removed extra /worksheets/
def download_worksheet(worksheet_id: int):
    """Get the Supabase URL for the worksheet file"""
    conn = get_connection()
    cur = conn.cursor()
    
    # Same bucket constant for consistency
    TARGET_BUCKET = "worksheets"
    ...
    try:
        ...
        if template_path and template_path.startswith('http'):
            # Optional: Verify the URL is using correct bucket
            # This helps catch any legacy URLs with wrong bucket names
            if '/projects/' in template_path:
                # Fix legacy URL
                template_path = template_path.replace('/projects/', f'/{TARGET_BUCKET}/')
            elif '/worksheets/worksheets/' in template_path:
                # Fix double folder nesting
                template_path = template_path.replace('/worksheets/worksheets/', f'/{TARGET_BUCKET}/')
